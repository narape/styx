...
public class DatastoreStorageTransaction implements StorageTransaction {
  ...
  private List<String> dafuqWeDid = Lists.newArrayList();
  ...
  public void commit() throws TransactionException {
    try {
      tx.commit();
      LOG.info("Committed {}", dafuqWeDid);
    } catch (DatastoreException e) {
      LOG.info("Failed commit {}, {}", dafuqWeDid, e);
  ...
  public void rollback() throws TransactionException {
    try {
      tx.rollback();
      LOG.info("Rolled back {}", dafuqWeDid);
    } catch (DatastoreException e) {
      LOG.info("Failed rollback {}, {}", dafuqWeDid, e);
      throw new TransactionException(e);
    }
  }
  ...
  public void store(Shard shard) {
    dafuqWeDid.add("store: " + shard.counterId() + "-" + shard.index() + " -> " + shard.value());
  ...
  public WorkflowInstance writeActiveState(WorkflowInstance instance, RunState state)
      throws IOException {
    dafuqWeDid.add("writeActiveState: " + instance + " " + state);
  ...
  public WorkflowInstance updateActiveState(WorkflowInstance instance, RunState state)
      throws IOException {
    dafuqWeDid.add("updateActiveState: " + instance + " " + state);
